      ...
      <div class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 transition-all hover:shadow-md">
          ...
          <form @submit.prevent="salvarUnidade" class="flex gap-3">
            <input v-model="novaUnidade.nome" type="text" required placeholder="Ex: Preparação de Massa" class="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <ul class="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
            ...
            <li v-for="u in unidades" :key="u.id" class="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
              ...
              <button @click="deletarUnidade(u.id)" class="text-slate-300 hover:text-rose-500 transition-colors" title="Excluir Unidade">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 transition-all hover:shadow-md">
          ...
          <form @submit.prevent="salvarEquipe" class="space-y-4">
            <div>
              ...
              <select v-model="novaEquipe.unidade_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-500 text-sm bg-white">
                <option value="" disabled>Selecione de qual unidade é esta equipe...</option>
                <option v-for="u in unidades" :key="u.id" :value="u.id">{{ u.nome }}</option>
              </select>
            </div>
            <div class="flex gap-3">
              <input v-model="novaEquipe.nome" type="text" required placeholder="Ex: Turma A - Manhã" class="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-500 text-sm">
              <button type="submit" :disabled="loadingEquipe || !novaEquipe.unidade_id" class="bg-emerald-600 text-white font-bold py-2.5 px-6 rounded-xl hover:bg-emerald-700 transition-all disabled:opacity-50 whitespace-nowrap">
                Vincular
              </button>
            </div>
          </form>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <ul class="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
            ...
            <li v-for="eq in equipes" :key="eq.id" class="p-4 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-slate-50 transition-colors group">
              ...
              <button @click="deletarEquipe(eq.id)" class="text-slate-300 hover:text-rose-500 transition-colors mt-2 sm:mt-0 opacity-0 group-hover:opacity-100" title="Excluir Equipe">
                <i class="fa-solid fa-trash-can"></i>
              </button>
            </li>
          </ul>
...
const fetchData = async () => {
  ...
  // Busca Equipes trazendo junto o nome da Unidade pai
...
const salvarEquipe = async () => {
  ...
  const { error } = await supabase.from('equipes').insert([
    { nome: novaEquipe.value.nome, unidade_id: novaEquipe.value.unidade_id }
  ])
  if (!error) {
...
const deletarUnidade = async (id) => {
  if(confirm('Tem certeza? Isso excluirá as equipes vinculadas a ela.')) {
