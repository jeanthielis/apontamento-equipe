    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <button v-for="cat in categorias" :key="cat.id" @click="filtroAtual = cat.id" :class="filtroAtual === cat.id ? cat.cor.replace('bg-', 'bg-opacity-100 bg-').replace('text-', 'text-white border-') : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'" class="px-4 py-2 rounded-full text-xs font-bold border transition-colors shadow-sm">
            {{ cat.nome }}
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div v-for="tema in temasFiltrados" :key="tema.id" class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow relative group">
            <div class="flex justify-between items-start mb-3">
              ...
              <button @click="deletarTema(tema.id)" class="text-slate-300 hover:text-rose-500 transition-colors opacity-0 group-hover:opacity-100">
...
const fetchData = async () => {
  // Busca Categorias
  const { data: cats } = await supabase.from('dds_categorias').select('*').order('nome')
  if (cats) categorias.value = cats

  // Busca Temas
  const { data: tms } = await supabase.from('dds_temas').select('*').order('created_at', { ascending: false })
...
const getCategoriaCor = (id) => {
  const cat = categorias.value.find(c => c.id === id)
  return cat ? cat.cor : 'bg-slate-100 text-slate-600 border-slate-200'
}
...
const salvarTema = async () => {
  ...
  if (!error) {
    ...
    alert("Erro ao salvar o DDS: " + error.message)
  }
  loading.value = false
}

const deletarTema = async (id) => {
  if(confirm('Deseja excluir este tema da biblioteca?')) {
    await supabase.from('dds_temas').delete().eq('id', id)
    await fetchData()
  }
}
...
<style scoped>
/* Um pequeno truque de CSS para cortar o texto (line-clamp) se ele for muito grande no card */
.line-clamp-3 {
  display: -webkit-box;
  -webkit-line-clamp: 3;
